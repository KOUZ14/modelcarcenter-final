...
export default function SellerTermsPage() {
  return <PolicyPage title="Seller Terms" intro="These Seller Terms apply to professional stores and individual collectors who apply, create listings, or sell through Model Car Center.">
    ...
    <h2 id="fees">10. Fees and payouts</h2>
    ...
    <p><b>Sellers pay both the marketplace commission and the actual Stripe payment-processing fee.</b> Both are deducted automatically from your order proceeds. The processing fee has no Model Car Center markup and is based on the full customer payment, including shipping and sales tax. Its amount varies by payment method, card, and applicable Stripe pricing; a single fixed processing rate is not guaranteed. Model Car Center covers its Stripe Tax service fees and Stripe Connect account and payout fees.</p>
    <p><b>Your proceeds before fulfillment costs = item subtotal + buyer-paid shipping − marketplace commission − actual payment processing.</b> Collected sales tax is withheld and is not seller income or part of your payout. You remain responsible for postage, packaging, inventory, and other fulfillment costs. Completed order records itemize the customer payment, tax withheld, commission, processing deduction, seller proceeds, and payout status. If Stripe has not yet supplied the actual processing fee, proceeds remain pending until it is available. A processing deduction cannot exceed the proceeds available on that order.</p>
    <p>For orders under this fee policy, refunds reduce seller proceeds in proportion to the amount refunded out of the original customer payment. The commission and seller processing deduction are credited in that same proportion; on a full refund, seller proceeds are zero. Model Car Center covers any processing cost Stripe does not return for the refunded portion. Transfers already released may be reversed accordingly. Orders and checkout sessions created under an earlier fee policy retain that policy. There are no listing fees, monthly selling fees, marketplace subscription fees, or account-opening fees for V1.</p>
    <p>The marketplace fee is earned when the order is paid. Seller proceeds become eligible for transfer {REFUND_REQUEST_DAYS_AFTER_DELIVERY} calendar days after carrier-confirmed delivery. A return or refund case opened on time pauses transfer until final resolution. After release to the connected Stripe account, Stripe controls bank-payout availability and timing. Amounts transferred or paid out may be reduced by refunds, reversals, disputes, negative balances, legally required withholding, and other adjustments attributable to your transactions. You are responsible for reviewing transaction records and reporting an error promptly.</p>
