...
export function SellerInventoryUpload({ disabled, action }: { disabled: boolean; action: Action }) {
...
export function SellerBulkStock({ products, disabled, action }: { products: Array<{ id: string; sellerSku: string; title: string; priceCents: number; inventoryQuantity: number; reservedQuantity: number; availabilityType: string }>; disabled: boolean; action: Action }) {
  ...
  return <details className="store-panel seller-bulk-stock"><summary>Update stock &amp; price</summary><p>Sold a model elsewhere? Reduce its stock here. There is no automatic stock synchronization. Total stock includes reserved units and cannot be lower than those reservations. New changes apply to future purchases.</p>{rows.length ? <><div className="admin-table-wrap"><table><thead><tr><th>Model / SKU</th><th>Price (USD)</th><th>Total stock</th><th>Reserved</th></tr></thead><tbody>{rows.map(product => { const value = changes[product.id] ?? { price: (product.priceCents / 100).toFixed(2), inventoryQuantity: String(product.inventoryQuantity) }; return <tr key={product.id}><td><b>{product.title}</b><small>{product.sellerSku}</small></td><td><input aria-label={`Price for ${product.sellerSku}`} type="number" min="0" step="0.01" disabled={disabled || busy} value={value.price} onChange={event => setChanges(previous => ({ ...previous, [product.id]: { ...value, price: event.target.value } }))} /></td><td><input aria-label={`Total stock for ${product.sellerSku}`} type="number" min={product.reservedQuantity} step="1" disabled={disabled || busy} value={value.inventoryQuantity} onChange={event => setChanges(previous => ({ ...previous, [product.id]: { ...value, inventoryQuantity: event.target.value } }))} /></td><td>{product.reservedQuantity}</td></tr>; })}</tbody></table></div><button className="button dark small" disabled={disabled || busy || !Object.keys(changes).length} onClick={() => void save()}>{busy ? "Saving…" : `Save ${Object.keys(changes).length} changed items`}</button>{!Object.keys(changes).length && <p>Change a price or stock quantity to enable saving.</p>}</> : <p>Add or upload in-stock models first. Preorder quantities are managed in Preorders.</p>}{error && <p className="form-error" role="alert">{error}</p>}</details>;
}
