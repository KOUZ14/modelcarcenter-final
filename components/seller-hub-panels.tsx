...
export function HubOverview({ data, navigate }: Props) {
  ...
  return <div className="store-stack">
    <Heading eyebrow="Seller Hub / Overview" title="Your business, in focus." description={`Welcome back, ${data.store.contactName.split(" ")[0]}. Here’s where your store stands.`} action={<button className="button dark small" disabled={data.store.status === "suspended"} onClick={() => navigate("inventory", "all", "new")}>+ Add product</button>} />
    ...
    <section className="hub-opportunities-section" aria-labelledby="overview-opportunities">
      <div className="panel-heading"><div><p className="eyebrow">Your next move</p><h3 id="overview-opportunities">Turn insights into action.</h3></div><button className="text-button" onClick={() => navigate("opportunities")}>All opportunities →</button></div>
      <OpportunityCards data={data} navigate={navigate} />
    </section>
    <div className="hub-two-column">
      <section className="store-panel"><p className="eyebrow">Daily operations</p><h3>Keep things moving</h3><div className="hub-action-list">
      ...
      <section className="store-panel"><div className="panel-heading"><div><p className="eyebrow">Recent activity</p><h3>Latest orders</h3></div><button className="text-button" onClick={() => navigate("orders", "all")}>View all →</button></div>
        {data.orders.length ? <div className="hub-recent-orders">{data.orders.slice(0, 4).map((order) => <div key={order.id}><div><b>{order.orderNumber}</b><small>{formatUtcDate(order.createdAt)} · {order.fulfillmentStatus.replaceAll("_", " ")}</small></div><b>{formatMoney(order.totalCents, order.currency)}</b></div>)}</div> : <p className="store-empty">Your first order starts the story. Add inventory to get your store ready.</p>}
      </section>
    </div>
  </div>;
}
...
export function HubOpportunities({ data, navigate }: Props) {
  return <div className="store-stack">
    <Heading eyebrow="Seller Hub / Opportunities" title="Know your next move." description="Act on buyer interest, free up slow inventory, and stay ahead of demand." />
    <OpportunityCards data={data} navigate={navigate} />
    <section className="store-panel"><p className="eyebrow">Inventory health</p><h3>A queue for every next step</h3><div className="hub-action-list">{inventoryViews.slice(1).map(([value, label]) => <button key={value} onClick={() => navigate("inventory", value)}><span>{label}</span><b>{data.inventory.filter((row) => matchesInventoryView(row, value, data.hub.slowIds)).length} listings →</b></button>)}</div></section>
...
export function HubDemand({ data, navigate, filter }: Props & { filter?: string }) {
  ...
  return <div className="store-stack">
    <Heading eyebrow="Seller Hub / Demand" title="Listen to the market." description="See what collectors save, what they’re hunting for, and where your inventory fits." />
...
export function HubMarketing({ data, navigate, filter }: Props & { filter?: string }) {
  ...
  return <div className="store-stack">
    <Heading eyebrow="Seller Hub / Marketing" title="Give your inventory a lift." description="Use collector interest to decide which listings need attention." />
...
export function HubAnalytics({ data, navigate, children }: Props & { children: ReactNode }) {
  const { hub } = data;
  return <div className="store-stack">
    <Heading eyebrow="Seller Hub / Analytics" title="Understand what sells." description="Sales performance, buyer loyalty, and the health of your inventory." />
    ...
    <section className="store-panel" id="payouts"><p className="eyebrow">Payouts</p><h3>Follow your proceeds</h3><div className="hub-payouts"><div><span>Released to Stripe</span><b>{formatMoney(hub.releasedCents)}</b><small>Recorded transfers less reversals</small></div><div><span>Held proceeds</span><b>{formatMoney(hub.heldCents)}</b><small>{hub.pendingFeeOrders ? `${hub.pendingFeeOrders} additional orders awaiting final fee amounts` : "Orders awaiting release"}</small></div></div><p className="hub-note">Transfers are releases to Stripe, not confirmation of bank deposits. Legacy direct payouts are excluded. Review individual orders for holds, refunds, and release timing.</p><button className="text-button" onClick={() => navigate("orders", "all")}>Review order payouts →</button></section>
