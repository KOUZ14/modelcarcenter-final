...
export function ModelHuntForm({ compact = false }: { compact?: boolean }) {
  ...
  if (state.kind === "success") return <div className="success-message" role="status"><Icon name="check"/><h3>Your Model Hunt is active.</h3><p>{state.message}</p><p className="reference-code">Reference: {state.referenceCode}</p>{collector ? <Link className="text-link" href="/account?view=hunts">Track this Hunt in My Garage</Link> : <Link className="text-link" href="/sign-in?returnTo=/account?view=hunts">Create an account to track this Hunt</Link>}<button type="button" onClick={() => setState({ kind: "idle" })}>Start another hunt</button></div>;
  ...
  return <form className={`wanted-form ${compact ? "compact-form" : ""}`} onSubmit={submit}>
