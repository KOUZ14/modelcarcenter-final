...
export function CartPage({ automaticTax, taxBehavior }: { automaticTax: boolean; taxBehavior: "exclusive" | "inclusive" }) {
  ...
  async function checkout() {
    ...
    try {
      ...
      const response = await fetch("/api/checkout", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ policyVersion: POLICY_VERSION, destination, previousReservationId, items: selected.flatMap((group) => requestedItems(group.items)), sellerSelections: selected.map((group) => ({ sellerId: group.sellerId, shippingSelection: shippingFor(group).selection })) }) });
  ...
  return <div>
    ...
    <div className="cart-layout"><div className="cart-details">
      ...
      {selected.some((group) => group.items.some((item) => item.availabilityType === "preorder")) && <p className="checkout-preorder-notice">Preorders are charged in full today. Each seller’s order ships after its latest expected release date. Release dates may change.</p>}
      <label className="consent-check checkout-consent"><input type="checkbox" checked={acceptedPolicies} disabled={loading} onChange={(event) => setAcceptedPolicies(event.target.checked)}/><span>I agree to the <Link href="/terms">Marketplace Terms</Link> and acknowledge the <Link href="/privacy">Privacy Policy</Link>, <Link href="/shipping">Shipping Policy</Link>, and <Link href="/returns">Returns &amp; Refunds Policy</Link>.</span></label>
