...
Preorders are created directly in the listing form by selecting **Preorder · 10% deposit**. Sellers enter the expected ship date, incoming quantity and buyer limit. Buyers pay 10% upfront, then the remaining balance when stock is ready. Deposits are non-refundable for a change of mind unless the seller approves a refund; inability to fulfill and qualifying delays require refunds. `/store/preorders` manages incoming stock, date updates and customer refunds. See [Preorders](preorders.md) for the complete flow and migrations `0032`–`0034`.
