...
- [ ] **Customer Support / Resolution Center** — `/resolution` — [Live](https://modelcarcenter.com/resolution) · [Local](http://127.0.0.1:5173/resolution). **Account.** Review eligible orders, opening a case, case details, attachments, responses, and status. Open real test records to exercise `?order=...` and `?case=...`.
