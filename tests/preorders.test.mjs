...
test('preorder database and checkout lifecycle',async t=>{
  ...
  // D1 limits result sets to 100 columns, including columns from joined tables.
  const prepare=query=>{const statement=sqlite.prepare(query);assert.ok(statement.columns().length<=100,'D1 result set exceeds 100 columns');return statement;};
  const binding={prepare(query){let values=[];return {bind(...args){values=args;return this;},async raw(){const s=prepare(query);s.setReturnArrays(true);return s.all(...values);},async all(){return {results:prepare(query).all(...values),success:true};},async first(column){const row=prepare(query).get(...values);return column?row?.[column]??null:row??null;},async run(){const r=prepare(query).run(...values);return {success:true,meta:{changes:r.changes}};}};},async batch(statements){sqlite.exec('BEGIN');try{const results=[];for(const s of statements)results.push(await s.run());sqlite.exec('COMMIT');return results;}catch(e){sqlite.exec('ROLLBACK');throw e;}}};
