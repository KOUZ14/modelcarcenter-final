...
test("consolidated payments, quotes, inventory, refunds, and payouts use distinct seller orders", async (t) => {
  ...
  for (const seller of ["a", "b"]) {
    ...
    sqlite.prepare(`INSERT INTO products (id, seller_id, slug, seller_sku, title, scale, model_manufacturer, vehicle_make, vehicle_model, price_cents, inventory_quantity, status) VALUES (?, ?, ?, ?, ?, '1:18', 'Maker', 'Make', 'Model', ?, 50, 'active')`).run(seller, seller, seller, seller, `Model ${seller}`, seller === "a" ? 10000 : 8000);
  }
